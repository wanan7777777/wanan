...
(() => {
  ...
  if (typeIndex === -1) {
    ...
    let t = +prompt(
      "当前域名无法执行脚本,输入1转跳职教云,输入2转跳智慧职教,其他取消转跳"
    ); //转number型
    ...
    setTimeOut(async () => {
      ...
      let res = await _ajax(
        typeIndex
          ? "/portal/LoginMooc/getUserInfo"
          : "/api/student/Studio/index"
      ); //查询用户信息
      if (res == null || res.code == -1) {
        ...
        $c_left
          ...
          .attr(
            "src",
            localStorage.getItem("avator").split("?")[0] +
              "?x-oss-process=image/resize,m_fixed,w_110,h_110,limit_0"
          );
        $c_left.find(".user-name").text(name);
        $c_left.find(".stuNum").text(localStorage.getItem("userName"));
        Console(
          `[${name}]用户您好，欢迎━(*｀∀´*)ノ亻!使用本脚本，该脚本已更新为2.0版本`
        );
        Console(`如在使用过程中出现BUG等情况,可联系邮箱反馈给作者`);
        if (typeIndex)
          Console(
            `该脚本不支持做测验题,所以会出现课程未完成但没办法全部完成子节点情况，是因为跳过了测验题，建议手动完成测验题再执行该脚本或者忽视测验题`
          );
        config._Lock = false;
      }
    });
    class _script {
      ...
      init(type) {
        ...
        this.url.courseLists = type
          ? "/portal/Course/getMyCourse?isFinished=0"
          : "/api/student/learning/getLearnningCourseList"; //课程列表
        this.url.moduleLists = type
          ? "/study/learn/getProcessList"
          : "/api/study/process/getProcessList"; //课程模块列表
        this.url.nodeLists = type
          ? "/study/learn/getTopicByModuleId"
          : "/api/study/process/getTopicByModuleId"; //模块节点列表
        this.url.childNodeLists = type
          ? "/study/learn/getCellByTopicId"
          : "/api/study/process/getCellByTopicId"; //模块子节点列表
        this.url.childNodeInfo = type
          ? "/study/learn/viewDirectory"
          : "/api/common/Directory/viewDirectory"; //模块子节点信息
        this.url.setProgress = type
          ? "/study/learn/statStuProcessCellLogAndTimeLong"
          : "/api/common/Directory/stuProcessCellLog"; // 修改进度
        this.url.nodeDataChange = type
          ? "/study/learn/computatlearningTimeLong"
          : "/api/common/Directory/changeStuStudyProcessCellData"; //节点数据改变
      }
        ...
        while (mI < mL) {
          ...
          while (tI < tL) {
            ...
            if (node != null && node.length == 0) {
              ...
              if (config.close) break;
              CourseList[i].module[mI].topic[tI].Nodes =
                res.data.length == 0 ? null : res.data;
    ...
    async function getChildNodeInfo() {
      try {
        ...
        while (unNodeList != 0) {
          ...
          let arr = v.split("-");
          Console(
            `当前子节点信息为${+arr[0] + 1}-${+arr[1] + 1}-${+arr[2] + 1}节点`
          );
          let node =
            CourseList[config.index[0]].module[arr[0]].topic[arr[1]].Nodes[
              arr[2]
            ];
          let isJump = false,
            JumpTxt = "";
    ...
    async function SetProgress(res, node) {
      try {
        ...
        let obj = $Script.filterNeedData(res),
          ...
          request = null;
        Console(
          `当前小节 类型:[${obj.info.type}] 名称:[${obj.info.name}] 长度:[${len}]`
        );
        if (!obj.info.is) {
          ...
          if ($Script.type) {
            ...
            for (let i = 1; i <= sum; i++) {
              ...
              $Script.type
                ? (obj.data.auvideoLength = time)
                : (obj.data.studyNewlyTime = time);
              if (config.close) break;
              request = await _ajax($Script.url.setProgress, obj.data);
              if (request.code >= 1) {
                ...
                if (request.code == -100) {
                  ...
                  Console(
                    `修改失败！错误码为${request.code},错误信息${request.msg}`
                  );
                  Console(`正在恢复默认速度,并进行重试`);
                  $("#video-set").val(
                    (config.ajaxSpeed = config.videoRequestSpeed = 10000) / 1000
                  );
                  ...
                  if (config.errorNum > 3) {
                    Console(
                      `连续异常3次已暂停,如有重复异常过多,可刷新页面重新运行该脚本`
                    );
                    $run.click();
                  }
                }
        ...
        for (const e of v.topic) {
          ...
          if (e.Nodes != null) {
            for (const r of e.Nodes) {
              html += `
                        <li class="view-3 ${r.unNum ? "" : "isOk"}" data-un=${
                r.unNum
              } >
                        <b>${r.type}</b>
                        <span>${r.name}</span>
    ...
    function userInit() {
      ...
      if (localStorage.getItem("scriptID") !== id) {
        ...
        Console("对运行环境数据初始化中。。。");
        if (localStorage.getItem("s_courseList"))
      ...
      if (config.isPause === false) {
        ...
        setTimeOut(() => {
          if (config.errorNum > 3) {
            ...
            Console(
              `失败原因可能为[登录状态失效，网络异常，账户信息异常]，建议刷新本页面成功后再重新执行该脚本`
            );
            Console(`正在尝试重新执行`);
            clearTimeout(config.timeOut);
